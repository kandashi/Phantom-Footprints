...
class PF {
    ...
    static PFSetup(){
    this.maxSteps = parseInt(game.settings.get("Phantom-Footprints", "footstep-maxIcon"))
    ...
    static async CheckPlacement() {
        ...
        this.StepLevel = this.maxSteps - parseInt(this.PerDif / this.footStep)
        return;
    }
    ...
    static async CreateSprite() {
        ...
        const texture = this.token.isVisible ? await loadTexture(PF.FootTexturePath) : await loadTexture(PF.PingTexturePath)
    ...
    static SetArray() {
        ...
        if (StepArray.length > this.maxSteps) {
